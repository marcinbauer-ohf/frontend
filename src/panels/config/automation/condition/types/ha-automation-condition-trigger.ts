...
export class HaTriggerCondition extends LitElement {
  ...
  protected render() {
    ...
    return html`
      ...
      <ha-list-selectable
        @ha-list-item-selected=${this._optionSelected}
        @ha-list-item-deselected=${this._optionDeselected}
        multi
      >
        ${this._renderOptions(
          selectedIds,
  ...
  private _optionSelected(ev: CustomEvent<number>) {
    this._toggleTriggerId(ev, true);
  }

  private _optionDeselected(ev: CustomEvent<number>) {
    this._toggleTriggerId(ev, false);
  }

  private _toggleTriggerId(ev: CustomEvent<number>, selected: boolean) {
    ev.stopPropagation();
    const triggerId = (
      (ev.currentTarget as HaListSelectable).items[
        ev.detail
      ] as HaListItemOption
    )?.value;
    if (!triggerId) {
    ...
    if (selected) {
      ...
      const index = ids.indexOf(triggerId);
      if (index === -1) {
        return;
      }
      ids.splice(index, 1);
