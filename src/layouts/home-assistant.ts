...
import { blockGuardedPop, navigate } from "../common/navigate";
...
export class HomeAssistantAppEl extends QuickBarMixin(HassElement) {
  ...
  protected firstUpdated(changedProps: PropertyValues<this>) {
    ...
    window.addEventListener("popstate", () => {
      // A page with unsaved changes puts the popped entry back and prompts
      // instead, so the route must stay where it is.
      if (blockGuardedPop()) {
        return;
      }
      updateRoute();
