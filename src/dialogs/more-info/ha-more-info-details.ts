...
interface DetailEntry {
  ...
  value: string;
  href?: string;
  /** Trailing icon for linked rows, must carry `slot="end"`. */
  icon?: TemplateResult;
  copyable?: boolean;
}
...
class HaMoreInfoDetails extends LitElement {
  ...
  private async _copyValue(ev: Event) {
    const { value } = ev.currentTarget as HTMLElement & { value: string };
  ...
  static styles: CSSResultGroup = css`
    ...
    /* Mirrors ha-list-item-value, which cannot be interactive itself. */
    ha-list-item-button {
