...
const formatDeviceClass = (deviceClass: string): string =>
  deviceClass
    .split("_")
      ...
      return Array.from(deviceClasses.values())
        .filter(
          (deviceClass) =>
            ...
            formatDeviceClass(deviceClass).toLowerCase().includes(filter)
        )
        .sort((a, b) =>
          stringCompare(formatDeviceClass(a), formatDeviceClass(b), language)
        );
    }
  );
